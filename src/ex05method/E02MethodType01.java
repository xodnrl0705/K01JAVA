...
Method(메소드)
	...
	즉 어떤 하나의 업무를 처리하는 모듈(부속품)이라 정의할수 있다.
규칙
...
Java에서의 Naming Rule(이름규칙)
	...
	-상수 : SIMPLE_FUNC -> 전체를 대문자로 표현하고, 연결된
		단어가 있는경우 _(언더바)를 사용한다.
	-패키지명 : simplefunc -> 전체를 소문자로 표현한다. 연결된
		단어가 있더라도 언더바를 사용하지 않는다. 언더바 대신 .(닷)
		으로 표현한다.
 */
...
public class E02MethodType01 {
	 ...
	 static void noParamNoReturn3() {
		 ...
		 //return;
		 /*
		 위와같이 함수 중간에 return을 쓰게되면 함수가 종료 되므로
		 아래 문장은 실행되지 않는 쓰레기 코드가 된다.
		 따라서 return을 사용할때는 반드시 조건문과 같이 사용해야 한다.
