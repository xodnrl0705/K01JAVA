...
public class E01JungsuType {
	...
	public static void main(String[] args) {
		...
		-큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형변환된다.(데이터의
		손실이 있기때문)
		-단 byte와 short는 예외적인 상황으로 생각하면 된다.(int형 연산을
		좋아하는 CPU의 특성)
		 */
		
		long long1 = 1000, long2 = 2000;
		long result4 = int1 + long2;//int + long = long형으로 변환
		System.out.println("int1 + long1 = " + result4);
		
		result4 = long1 + long2;//long끼리의 연산은 기본규칙을 따른다.
		System.out.println("long1 + long2 = " + result4);
		
		//long long3 = 22000000000; [에러발생]
		long long3 = 22000000000L; 
		long long4 = 22000000000l;/*
		 	자바컴파일러(CPU)는 정수형상수를 기본적으로 int형으로
		 	간주하기때문에 위 정수를 메모리에 올리는 순간 에러가 발생한다.
		 	이때는 접미사(L or l)을 붙여서 int형 자료가 아닌
		 	long형 자료라는것을 알려줘야 에러가 발생하지 않는다.
