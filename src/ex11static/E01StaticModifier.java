...
접근방법
	...
	-클래스외부
		...
		(참조변수로 접근하는것은 java에서 권장하지 않는다.)
 */
class MyStatic{
	/*
	[멤버변수] 인스턴스형 멤버변수는 반드시 클래스의 객체를
	생성한후 참조변수를 통해 접근해야한다.
	 */
	int instanceVar; 
	/*
	[정적멤버변수] 객체를 생성할 필요없이 클래스명으로 바로
	접근이 가능하다. 프로그램 시작시 이미 메소드영역에
	초기화 된다.
	 */
	...
	(※main메소드에서는 static으로 선언된 메소드만 호출할수
	있었음을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		방법2 : 인스턴스변수처럼 참조변수를 통해 접근할수 있다.
			단, 이경우는 정적변수를 인스턴스변수로 착각할수 
			있으므로 권장하지 않는다.
		 */
		...
		/*
		인스턴스 멤버변수는 객체별로 각각 존재하므로 아래 초기화된
		값은 객체별로 각각 저장된다. 따라서 출력결과는 100, 1000
		이 된다.
		 */
		ms1.instanceVar = 100;
		...
		메소드영역에 하나만 생성되어 모든 객체가 공유하는 공유변수가된다.
		...
		*/
		...
		/*
		정적 멤버메소드의 접근방법도 정적 변수와 동일한 규칙을
		가진다.
		 */
