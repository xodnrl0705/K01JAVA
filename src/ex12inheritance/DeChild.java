...
public class DeChild extends DeParent {
	...
	/*
	오버라이딩 아님.
	부모클래스에서 private멤버메소드로 선언되었기
	때문에 자식에서는 보이지 않으므로 오버라이딩의
	대상이 될수 없다. 해당 함수는 자식에서 새롭게
	확장한 메소드가 된다.
	 */
	private void eat() {
	...
	/*
	메소드 오버라이딩의 조건
	-부모클래스에서 선언될때와 비교해서 접근범위가
	동일하거나 그보다 넓으면 가능하다.
	-즉 부모에서 default면 자식쪽에서 default,
	public이면 오버라이딩이 가능하다.
	 */
	@Override
	public String sleep() {
	...
	super.멤버 : 부모클래스에 정의된 멤버변수 혹은 멤버메소드를
		호출할때 사용
	 */
	@Override
