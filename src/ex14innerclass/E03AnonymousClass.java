...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		부모클래스의 참조변수로 참조함.
		참조변수가 부모타입이므로 자식객체에 직접 접근하는것은 불가능.
		반드시 다운캐스팅 후 접근해야 한다.
		 */
		...
		/*
		Person클래스를 상속받은 익명클래스 정의및 생성
		: Person 타입의 인스턴스 변수에 Person을 상속받은 익명클래스를
		생성해서 그 주소를 할당한다.
		개념적으로 new Person(){}; ==> 익명 extends Person{};
		형태라 생각하면 된다.
		이름이 없는 객체이므로 부모클래스의 이름을 빌려서 생성했다고
		생각하자.
		 */
		...
		/*
		부모타입의 참조변수로 하위객체에서 확장한 멤버에 접근할수 없으므로
		위 부분은 에러가 발생한다.
		 */
		...
		/*
		하위 익명클래스에서 확장한 변수이므로 익명객체를 통해서는
		접근불가.
		 */
		...
		/*
		다운캐스팅은 "(자식클래스명)부모타입의 참조변수" 형태로
		형변환하여 하위클래스의 멤버에 접근하기 위한 수단이다.
		그러나 익명클래스는 클래스명이 없기때문에 위와같이 다운캐스팅이
		불가능하다.
		즉, 익명클래스에서는 새롭게 정의한 멤버변수를 접근할 수 없다.
		따라서 익명클래스는 오버라이딩의 목적으로만 사용해야한다. 
		 */
		/*
		Person anonyTest = new Student("다운캐스팅", "안되욥") {
			String jumin = "001024";
		};
