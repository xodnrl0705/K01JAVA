...
-정적 멤버변수 & 멤버 메소드를 가질 수 있다.
-외부클래스의 인스턴스형 멤버는 사용할수 없다.
-외부클래스명$내부클래스명.class 파일이 생성된다.

※원래 클래스 앞에는 static을 붙일수 없으나 내부클래스에는 허용된다.
 */
class OuterClass{
	...
	static void outerStaticMethod() {
		...
		/*
		외부클래스의 정적멤버 메소드에서는 내부클래스의 정적멤버만
...
public class E02InnerStaticClass {
	...
	public static void main(String[] args) {
		...
		/*
		내부정적 클래스의 인스턴스멤버에 접근할때는 객체를 생성한후
		참조변수를 통해서 접근해야 한다.
		 */
		...
		그외는 클래스 접근의 일반적인 방법을 따른다.
		 ...
		 외부클래스의 멤버변수(메소드)는 객체생성후 참조변수를 통해
		 접근한다.
		 */
