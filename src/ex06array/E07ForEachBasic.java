...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		배열 arr의 요소를 하나씩 참조하여 출력하고, 더이상 
		참조할 요소가 없을때 for문이 자동으로 종료된다.
		 */
		sum=0;
		for(int e : arr) {
			System.out.print(e +" ");
		...
		아래 증가 연산자를 통해 증가하는 것은 실제 배열에 적용되지
		않는다. 해당 foreach문 내에서만 증가하게 된다.
		 */
		...
		for(int c : arr) {
			System.out.println(c + " ");/* +1증가된 부분이 기존 배열에
								적용되지 않으므로 기존의 값이 그대로 출력된다.*/
		...
		/*
		2차원 배열은 1차원배열 여러개를 겹쳐놓은 형태이므로
		출력시에는 우선 하나의 행(1차원배열)을 가져와서...
		 */
		for(int[] a : twoDim) {
			//각 요소를 하나씩 참조하여 출력한다.
			for(int b : a) {
